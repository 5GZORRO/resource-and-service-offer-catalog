...
public class OrganizationController implements OrganizationInterface {
    ...
    @ApiOperation(value = "Creates a Organization", nickname = "createOrganization",
            notes = "This operation creates a Organization entity.", response = Organization.class, authorizations = {
    ...
    createOrganization(@ApiParam(value = "The Organization to be created", required = true )
                       @Valid @RequestBody OrganizationCreate organization) {
        ...
        Organization o;
        try {
            o = organizationService.create(organization);
        ...
        log.info("Web-Server: Organization created with id " + o.getId() + ".");

        return ResponseEntity.status(HttpStatus.CREATED).body(o);
    }

    @ApiOperation(value = "Deletes a Organization", nickname = "deleteOrganization",
    ...
    @ApiResponses(value = {
            ...
            //@ApiResponse(code = 409, message = "Conflict", response = Error.class),
            @ApiResponse(code = 500, message = "Internal Server Error", response = Error.class) })
    @RequestMapping(value = "/party/v4/organization/{id}",
            produces = { "application/json;charset=utf-8" },
            method = RequestMethod.DELETE)
    @ResponseStatus(value = HttpStatus.NO_CONTENT)
    public ResponseEntity<?>
    deleteOrganization(@ApiParam(value = "Identifier of the Organization", required = true)
                       @PathVariable("id") String id) {

        log.info("Web-Server: Received request to delete Organization with id " + id + ".");
        ...
        log.info("Web-Server: Organization " + id + " deleted.");

        return new ResponseEntity(HttpStatus.NO_CONTENT);
    }

    @ApiOperation(value = "Updates partially a Organization", nickname = "patchOrganization",
            notes = "This operation updates partially a Organization entity.", response = Organization.class,
            authorizations = {
            @Authorization(value = "spring_oauth", scopes = {
                    @AuthorizationScope(scope = "read", description = "for read operations"),
                    @AuthorizationScope(scope = "openapi", description = "Access openapi API"),
    ...
    public ResponseEntity<?>
    patchOrganization(@ApiParam(value = "Identifier of the Organization", required = true)
                      @PathVariable("id") String id,
                      @ApiParam(value = "The Organization to be updated" ,required = true )
                      @Valid @RequestBody OrganizationUpdate organization) {

        log.info("Web-Server: Received request to patch Organization with id " + id + ".");
        ...
        Organization o;
        try {
            o = organizationService.patch(id, organization);
        } catch (NotExistingEntityException e) {
            log.error("Web-Server: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ErrMsg(e.getMessage()));
        }

        log.info("Web-Server: Organization " + id + " patched.");

        return ResponseEntity.status(HttpStatus.OK).body(o);
    }

    @ApiOperation(value = "Retrieves a Organization by ID", nickname = "retrieveOrganization",
            notes = "This operation retrieves a Organization entity. Attribute selection is enabled for all first level attributes.",
            response = Organization.class, authorizations = {
            @Authorization(value = "spring_oauth", scopes = {
                    @AuthorizationScope(scope = "read", description = "for read operations"),
                    @AuthorizationScope(scope = "openapi", description = "Access openapi API"),
                    @AuthorizationScope(scope = "admin", description = "Access admin API"),
    ...
    public ResponseEntity<?>
    retrieveOrganization(@ApiParam(value = "Identifier of the Organization", required = true)
                         @PathVariable("id") String id,
                         @ApiParam(value = "Comma-separated properties to provide in response")
                         @Valid @RequestParam(value = "fields", required = false) String fields) {

        log.info("Web-Server: Received request to retrieve Organization with id " + id + ".");
        ...
        log.info("Web-Server: Organization " + id + " retrieved.");
